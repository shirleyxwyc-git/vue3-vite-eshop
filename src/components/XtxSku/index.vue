...
export default {
  name: 'XtxGoodSku',
  props: {
    // 有backend 傳入並接收名為goods的對象參數，包含屬性:1）specs:所有的規格信息  & 2）skus:所有的sku組合
    goods: {
      type: Object,
      default: () => ({ specs: [], skus: [] }),
    },
  ...
  setup(props, { emit }) {
    ...
    const clickSpecs = (item, val) => {
      ...
      if (selectedArr.length === props.goods.specs.length) {
        ...
        // 觸發名為change 的事件及拋出一個對象，傳遞數據給父組件， 供之後加到購物車使用
        emit('change', {
          skuId: sku.id,
          price: sku.price,
...
<template>
  <div class="goods-sku">
    <dl
      ...
      :key="item.id"
      v-show="item.values.findIndex((val) => val.picture) !== -1"
    >
      <dt>{{ toHK(item.name) }}</dt>
      <dd>
